...
% Generates FBA solution at each point on 2D parameter scan using 'Squeeze'
% to set parameters for degree of compression of expression values and
% 'Multiplier' to set the ratio between EX reaction bounds (linked to DMEM
% composition) and internal transport and metabolic reaction bounds (linked
% to expression data).
...
%
% OUTPUT = stored data and a #Squeeze * #Multiplier number of diagnostic
% spreadsheets

% The spreadsheets contain a large number of parameters from the FBA
% including limiting reactions and non-zero reactions linked to key
% metabolites

% There are various options set, not all of which are required for
% MEF-analysis
...
for SqueezeCount = 1:numel(Squeeze)
    ...
    % SUBSET AND SUM THE ATPASE ROWS FOR SAVING WITH MAIN DATAFILES
